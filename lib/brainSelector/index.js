  ...
  getBrainForInput (input) {
    ...
    return Promise.any(promises)
      .then((brain) => {
        ...
        this.lastSelectedBrainsPerClient[input.plugin] = {time: (new Date()).getTime() / 1000, brain: brain}
        return brain
      })
      .catch((err) => {
        if (this.lastSelectedBrainsPerClient[input.plugin]) {
          ...
          if (now - selectedInfo.time < this.brainStickiness) {
            selectedInfo.time = (new Date()).getTime() / 1000
            ...
            return Promise.resolve(selectedInfo.brain)
          }
        }
